...
const Home = () => {
  const { allUsers, loading: userLoading } = useSelector(
    (state) => state.allUserStore
  );
  const { allPosts, loading: postLoading } = useSelector(
  ...
  return loading || userLoading ? (
    ...
    <div className="home d-flex justify-content-around">
      ...
      <Card className="right d-flex flex-col">
        {allPosts?.length > 0 ? (
          allPosts?.map((item) => <Post key={item?._id} postItem={item} />)
        ) : (
